...
class AdvancedGPTToProlog:
    ...
    def get_related_topics(self, topic, max_topics=10):
        """Query GPT to get related topics for the main topic."""
        prompt = f"""
        ...
        try:
            ...
            return json_data.get("related_topics", [])
            
            
        except Exception as e:
            print(f"Error querying GPT for related topics: {e}")
            return []

    def get_concepts_from_gpt(self, topic, domain_type="general"):
        """
        Query GPT about concepts and relationships related to a topic.
        ...
        try:
            ...
            print(f"Error querying GPT about concepts: {e}")
            raise
    
    def _extract_json_from_response(self, content):
        """Helper method to extract JSON from GPT's response."""
            ...
            try:
                ...
                if json_start >= 0 and json_end > json_start:
                    try:
                        ...
                        raise ValueError("Could not extract valid JSON from GPT's response")
                else:
                    raise ValueError("Could not find JSON in GPT's response")
    
    def generate_prolog_facts(self, data, topic):
        """Convert the JSON data from GPT into Prolog predicates."""
    ...
    def build_knowledge_network(self, main_topic, max_related_topics=10, max_depth=2, output_file=None):
        ...
        while topic_queue:
            ...
            try:
                ...
                data = self.get_concepts_from_gpt(current_topic, domain_type=domain_type)
    ...
    def generate_knowledge_report(self, prolog_file, main_topic):
        ...
        # Create a prompt for GPT to generate the report
        prompt = f"""
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Build a comprehensive Prolog knowledge network from GPT")
    parser.add_argument("topic", help="Main topic to start the knowledge network")
    parser.add_argument("--api-key", help="GPT API key (if not set in environment)")
    ...
    extractor = AdvancedGPTToProlog(api_key=args.api_key)
